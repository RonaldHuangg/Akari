...
public class ModelImpl implements Model {
    ...
    public boolean isLit(int row, int col) {
        ...
        for (int i = row - 1; i >= 0; i--) {
            if (activePuzzle.getCellType(i, col) == CellType.WALL || activePuzzle.getCellType(i, col) == CellType.CLUE) {
                break;
            }
            if (isLamp(i, col)) {
                return true;
        ...
        for (int i = col - 1; i >= 0; i--) {
            if (activePuzzle.getCellType(row, i) == CellType.WALL || activePuzzle.getCellType(row, i) == CellType.CLUE) {
                break;
            }
            if (isLamp(row, i)) {
                return true;
    ...
    public void setActivePuzzleIndex(int index) {
        if (index < 0 || index >= library.size()) {
            throw new IndexOutOfBoundsException("The index must be within the bound of the puzzle library");
        }
        activeIndex = index;
        resetPuzzle();
    }
